...
func TestMutableNetworkBasicOperations(t *testing.T) {
	...
	// Test adding edges
	err := n.AddEdge("edge1", "A", "B")
	if err != nil {
		t.Errorf("Expected AddEdge to succeed, got error: %v", err)
	}
	err = n.AddEdge("edge1", "A", "B")
	if err != nil {
		t.Errorf("Expected AddEdge to succeed for existing edge, got error: %v", err)
	}
	...
	// Test incident nodes
	endpoints, err := n.IncidentNodes("edge1")
	if err != nil {
		t.Errorf("Expected IncidentNodes to succeed, got error: %v", err)
	}
	...
	predecessors, err := n.Predecessors("A")
	if err != nil {
	...
	// Test adjacent nodes
	adjacent, err := n.AdjacentNodes("B")
	if err != nil {
	...
	// Test incident edges
	incidentEdges, err := n.IncidentEdges("B")
	if err != nil {
...
func TestDirectedNetwork(t *testing.T) {
	...
	predecessors, err := n.Predecessors(2)
	if err != nil {
		t.Errorf("Expected Predecessors to succeed, got error: %v", err)
	}
	...
	inEdges, err := n.InEdges(2)
	if err != nil {
		t.Errorf("Expected InEdges to succeed, got error: %v", err)
	}
	if !inEdges.Contains("e1-2") {
		t.Error("Expected e1-2 to be in-edge of 2")
	}

	outEdges, err := n.OutEdges(2)
	if err != nil {
		t.Errorf("Expected OutEdges to succeed, got error: %v", err)
	}
...
func TestNetworkWithSelfLoops(t *testing.T) {
	...
	endpoints, err := n.IncidentNodes("self")
	if err != nil {
		t.Errorf("Expected IncidentNodes to succeed, got error: %v", err)
	}
...
func TestNetworkPanicOnSelfLoop(t *testing.T) {
	...
	err := n.AddEdge("self", "A", "A")
	if err == nil {
		t.Error("Expected error when adding self-loop to network that doesn't allow them")
	}
}
...
func TestNetworkPanicOnParallelEdges(t *testing.T) {
	...
	err := n.AddEdge("edge2", "A", "B")
	if err == nil {
		t.Error("Expected error when adding parallel edge to network that doesn't allow them")
	}
}
...
func TestNetworkAdjacentEdges(t *testing.T) {
	...
	adjacentEdges, err := n.AdjacentEdges("e1")
	if err != nil {
		t.Errorf("Expected AdjacentEdges to succeed, got error: %v", err)
	}
...
func TestNetworkAsGraphView(t *testing.T) {
	...
	// Test that PutEdge returns error (not supported in network graph view)
	err = graph.PutEdge("A", "C")
	if err == nil {
		t.Error("Expected error when calling PutEdge on network graph view")
	}
}
